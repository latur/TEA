...
function SamplesLoaded(){
	...
	// View mode
	$('.chr-view-mode .aslist').click(function(){
			Route("chr1:5000000-10000000");
	});
	$('.chr-view-mode .asline').click(function(){
	...
	for (var i = 0; i < n_file; i++){
		...
		for (var s in expData[name][f]){
			...
			if (x < last_x + 10){
					...
					if (y >= i*60 + extra){
							extra += 15;
							sample.attr("height", n_file*50 + extra);
					}
			} else y = i*60 + 30;
...
function ShowChromosome(name, start, end){
	...
	createSmallBwtWeb('gene_db', 'sml0', name.substr(3), 2*start - end, 2*end - start);
	load_detail_content(name, start, end);
