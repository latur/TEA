	...
	for (var i = 0; i < (n_file-1); i += 1){
		...
		for (var k = i+1; k < n_file; k += 1){
			...
			for (var chr in expData){
				...
				for (var p1 = 0; p1 < expData[chr][name].length; p1++){
					for (var p2 = 0; p2 < expData[chr][n].length; p2++){
						if (Math.abs(expData[chr][name][p1][0] - expData[chr][n][p2][0]) < 100 &&
							expData[chr][name][p1][2] == expData[chr][n][p2][2]){
								pair += 1;
							break;
						}
					}
		...
		for (var a = 0; a < 2; a += 1){
			...
			if (name.length == 1){
				...
				circos.append("line")
					...
					.attr("id", (tree[i].dis/max_len).toPrecision(3))
					.on("mouseover", function(){
						var id = this.id;
						$("body").append(function(){
							return $("<div/>")
								.attr("class", "pop_up")
								.attr("style", "height: 20px; font-size: 12px; text-align: center;z-index:10001;")
